...
async function deepseekDefaultsServer(): Promise<DeepSeekDefaultsServer> {
  ...
  const server = createServer((request: IncomingMessage, response: ServerResponse) => {
    ...
    request.on('end', () => {
      ...
      // The comments-only prefix must outlast the adapter's idle watchdog
      // (fixture sets 1000ms) so this snapshot proves comments rearm it;
      // 12 × 100ms leaves slack for the 2-core fork runner's delayed timers
      // that once let the old 3 × 60ms / 150ms pairing time out and retry.
      let keepAlives = 12
      const write = (): void => {
        if (keepAlives-- > 0) {
