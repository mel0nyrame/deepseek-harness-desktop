...
The `web`, `headless`, and `desktop` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `headless`: base + headless; `desktop`: base + web-app + desktop-app, which disables the browser transport rows and mounts the dedicated-child IPC carrier the Electron shell supervises). Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`.
