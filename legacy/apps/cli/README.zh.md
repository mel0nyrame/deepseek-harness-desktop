...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`headless` 和 `desktop` profile 在首次使用时会从随附模板自动初始化；其他任何 profile 都必须通过 `dsh plugin` 创建。`desktop` profile 组合了 Web 产品，但禁用其浏览器传输行，改用单一专属子进程 IPC 载体；Electron 开发 shell 通过 `pnpm run dev:desktop` 启动它。
