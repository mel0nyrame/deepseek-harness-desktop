...
The invoking directory is the default workspace root. The `web`, `headless`, and `desktop` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. The `desktop` profile composes the Web product with its browser transport rows disabled and one dedicated-child IPC carrier instead; the Electron development shell launches it through `pnpm run dev:desktop`.
