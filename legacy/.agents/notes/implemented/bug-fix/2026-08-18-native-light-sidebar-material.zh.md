...
- 聚焦测试覆盖 preload 转发、IPC 校验、偏好去重与透明玻璃 CSS。安装包的三次启动旅程还会从 Electron main 读取 `nativeTheme.themeSource`，固定验证 Dark、Light、System、持久化与“减少透明度”行为。当 macOS 启用“减少透明度”时，验收会在启动阶段以及原生主题更新重新发布系统事实后验证不透明 fallback 与可见的覆盖提示，然后只在验收窗口中恢复透明度，使同一次运行仍会覆盖原生玻璃材质。在关闭窗口前，验收会等待最终的 Light 选择写入 Host 设置文档，避免重开流程与异步设置写入发生竞态。
