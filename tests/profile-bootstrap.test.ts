...
describe('desktop profile composition', () => {
  it('loads the bootstrapped bundles and dumps the ordered real Loader entry tree', () => {
    ...
    expect(ids.slice(-4)).toEqual(['desktop-directory-picker', 'desktop-connection', 'desktop-native', 'desktop-ui'])
    ...
    expect(entries.find(entry => entry.id === 'desktop-directory-picker')?.name).toBe('@deepseek-ai/dsh-host-directory-picker-native')
