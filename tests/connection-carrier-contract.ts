...
interface CarrierContractApi {
  readonly sessions: {
    list(payload: object): Promise<{ readonly rpcId: string; readonly result: unknown }>
    history(payload: { readonly sessionId: never }): Promise<{ readonly result: unknown }>
    search(payload: { readonly query: string }, signal: AbortSignal): Promise<unknown>
  }
  readonly events: {
    mux(payload: object, signal: AbortSignal, onOpen?: () => void): AsyncIterable<{ readonly rpcId: string }>
    host(payload: object, signal: AbortSignal, onOpen?: () => void): AsyncIterable<{ readonly rpcId: string }>
  }
  respond(response: unknown): Promise<unknown>
}

interface CarrierContractConnection {
  start(
    sinks: { onConnected(description: unknown): void },
    config: { readonly streamOpenTimeoutMs: number },
  ): { stop(): void }
}
...
export interface CarrierContractHarness {
  readonly api: CarrierContractApi
...
export type ConnectionHandleFactory = (transport: {
  readonly api: CarrierContractApi
  readonly rpc: { call(): Promise<never> }
  readonly isLoopback: true
}) => CarrierContractConnection
...
function muxMessage(rpcId: string, lastSeq: number): Record<string, unknown> & { readonly rpcId: string } {
  return {
    type: 'server-request',
    rpcId,
...
function hostMessage(rpcId: string): Record<string, unknown> & { readonly rpcId: string } {
  return {
    type: 'server-request',
    rpcId,
...
export function runCarrierContract(
  name: string,
  make: CarrierContractFactory,
  createConnectionHandle: ConnectionHandleFactory,
): void {
    ...
    it('echoes a Host request rpcId in the Client response and bounds duplicate lifetime', async () => {
      await usingHarness(make, async (harness) => {
        ...
        const response = {
          ...
          result: {
            ok: true as const,
            value: { sessionId: 'contract-session', approvalId: 'approval-1', outcome: 'allowed-once' as const },
          },
        }
    ...
    it.each([
      ...
      await usingHarness(make, async (harness) => {
        ...
        const connection = createConnectionHandle({
          api: harness.api,
          rpc: { call: () => Promise.reject(new Error('not used by carrier contract')) },
          isLoopback: true,
        })
        const controller = connection.start({
    ...
    it('waits for unary reachability after both streams are open', async () => {
      await usingHarness(make, async (harness) => {
        ...
        try {
          ...
          harness.open('host')
          let settled = false
          void connected.promise.then(() => { settled = true })
          await new Promise<void>(resolve => setTimeout(resolve, 0))
