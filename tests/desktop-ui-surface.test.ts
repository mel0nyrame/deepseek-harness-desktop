...
describe('desktop UI surface contribution', () => {
  ...
  it('anchors drag, no-drag, compact chrome, focus, and material rules to documented slots', () => {
    ...
    expect(DESKTOP_SURFACE_CSS).toContain("[data-sidebar-header]")
    expect(DESKTOP_SURFACE_CSS).not.toContain("[data-slot='sidebar'] > div > :first-child")
    expect(DESKTOP_SURFACE_CSS).toContain("[data-sidebar-collapsed='true'] [data-slot='sidebar']")
    expect(DESKTOP_SURFACE_CSS).toContain(":has(> [data-slot='sidebar'])")
    expect(DESKTOP_SURFACE_CSS).toContain('overflow: hidden')
    expect(DESKTOP_SURFACE_CSS).toContain("[data-sidebar-collapsed='true'] [data-slot='conversation.session.header'] > header")
    expect(DESKTOP_SURFACE_CSS).toContain('padding-left: 120px')
    expect(DESKTOP_SURFACE_CSS).toContain('padding-left: 48px')
    expect(DESKTOP_SURFACE_CSS).toContain("[role='tablist']")
  })
})
