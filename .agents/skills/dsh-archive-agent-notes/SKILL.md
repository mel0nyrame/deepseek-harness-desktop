...
5. Re-record the sidecar hashes by hand (`git hash-object` both edited files) and write them into the triplet's `.i18n.yaml`. The automated archive verifier (`verify-archived-agent-notes`) returns with a later slice; until then the repository-layout test pins the lifecycle tree structure.
...
Run `pnpm run lint`, `git diff --check`, and the repository-layout test (`pnpm run test:layout`), which pins the notes lifecycle tree and the retained skill set; select any additional evidence through [dsh-pre-push-checks](../dsh-pre-push-checks/SKILL.md).
