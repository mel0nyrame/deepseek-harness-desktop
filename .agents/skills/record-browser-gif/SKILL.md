...
1. Require a clean worktree, record its exact commit with `git rev-parse HEAD`, then build that recorded tree with the repository's current build commands (see [AGENTS.md](../../../AGENTS.md#commands); with no build yet, record against the checked-out tree). A GIF recorded against another commit's build misattributes the evidence.
