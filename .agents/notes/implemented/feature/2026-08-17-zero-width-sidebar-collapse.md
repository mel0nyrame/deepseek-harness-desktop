...
- The conversation header exposes the `data-conversation-header` attribute hook; client CSS clears the reveal control (48px) and `DESKTOP_SURFACE_CSS` adds the macOS traffic-light cluster clearance (112px windowed, 48px full screen).
- `DESKTOP_SURFACE_CSS` drops the collapsed-rail rule and adds `[data-sidebar-reveal]` positioning (76px windowed, 12px full screen) plus the header clearance rules.
- Packaged acceptance (`--accept-native-window`) now drags the sidebar, collapses it, and asserts the zero-width track, reclaimed conversation width, reveal position in both window states, header clearance, and the restored dragged width; `--record-native-window` captures `sidebar-collapsed`, `fullscreen-collapsed`, and `sidebar-revealed` frames.
