...
- The evidence journey records display bounds and work area as diagnostics but does not derive `BrowserWindow` outer bounds from either value. Initial sizing and later wide resizes must reach the requested width and a height between the platform minimum and the requested height; macOS can report a stable native frame that is not an exact clamp of either display metric.
