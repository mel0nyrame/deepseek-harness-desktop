...
The desktop UI package remains an additive Client and Host contribution. It registers the window controls in `shell.overlay` and the glass preference in `settings.general.item`; the official sidebar continues to render its labels, counts, conversations, and actions. Stable slot attributes and desktop-owned body attributes drive a small CSS layer that marks the title row as draggable and all controls as no-drag. The collapsed reveal button remains outside the zero-width sidebar, including in native full screen.
...
One versioned patch to `@deepseek-ai/dsh-client-ui-layout@0.1.0-rc.8` changes the closed sidebar track from 56 pixels to zero, retains the last dragged width, and publishes stable collapsed and resolved-width frame state. The runtime manifest records its digest, upstream source, tests, and removal condition. The patch can be removed when the published layout package provides the same contract.

## Alternatives considered

**Copy the official sidebar and layout packages.** Rejected because it would duplicate labels, counts, interaction behavior, and future upstream changes. Slots and stable frame state are sufficient for the desktop-owned behavior.
...
On 2026-08-31, `pnpm run check` passed typecheck, lint, all workspace builds, and 122 tests in 21 files; the one Accessibility-permission-gated OS-pointer case was skipped by the default run. `DSH_DESKTOP_REQUIRE_OS_DRAG=1 pnpm exec vitest run tests/desktop-runtime.e2e.test.ts -t 'moves only the computed drag surface'` separately passed that real drag/no-drag case. `pnpm run test:layout` passed all seven repository-layout checks. The focused behavior is exercised by `tests/desktop-native-window.test.ts`, `tests/desktop-ui-surface.test.ts`, `tests/desktop-ui-runtime.test.ts`, `tests/desktop-ui-host.test.ts`, `tests/desktop-ui-client.test.ts`, `tests/desktop-layout-patch.test.ts`, `tests/desktop-sidebar-integration.test.ts`, `tests/desktop-ui-visual.e2e.test.ts`, and the real Electron journeys in `tests/desktop-runtime.e2e.test.ts`.
