...
桌面采用已批准的 A — Compact macOS 框架作为平台感知的窗口基础（Issue #32）。macOS 保留 `hiddenInset` 原生 traffic lights，并把原生窗口表面作为拖动区域；侧边栏第一行把侧边栏开关放在原生灯组紧右侧，DeepSeek Harness 字标位于下一行。renderer 不再使用合成的全宽标题区域或全局顶部内缩。紧凑头部是仅 macOS 的呈现：`DESKTOP_SURFACE_CSS` 在 `body[data-dsh-platform='darwin']` 下应用它，而 Web、Windows 与 Linux 保留原有侧边栏外壳，只保留现有 `desktopWindowOptions` 扩展边界，不提供占位式窗口 chrome。进入原生全屏时隐藏 traffic lights（通过显式 `setWindowButtonVisibility` 同步），并借助 `data-dsh-fullscreen` 将侧边栏控制行与字标行移到侧边栏左侧内容内缩位置。
...
- 窗口态控制行避开原生 traffic lights 灯组（`MACOS_CONTROL_ROW_INSET_PX`）；全屏时两行回到侧边栏左侧内容内缩位置。
