...
The main repository owns native CI and local publication commands. `Landlock Run` runs for relevant pull requests and `master` pushes and builds each platform on its matching native runner. A release operator builds each platform on its native architecture, transfers both binary payloads into one clean checkout, assembles and verifies the complete package family, packs immutable npm tarballs, installs and exercises those tarballs, and then runs the registry-aware publisher. Platform tarballs publish before the entry tarball that optionally depends on them. Publication uses `landlock-run-vX.Y.Z` tags so launcher releases cannot collide with other release families in the monorepo; prereleases use the npm `next` dist-tag.
...
- **Give the launcher the root DeepSeek Harness version and publish the complete monorepo recursively** — rejected because this change owns one three-package public family, not the independent `@deepseek-ai/dsh-*` baseline. The [artifact-first npm baseline proposal](../../rejected/process/2026-08-04-artifact-first-npm-baseline-publication.md) explicitly keeps native workspaces outside its target set.
