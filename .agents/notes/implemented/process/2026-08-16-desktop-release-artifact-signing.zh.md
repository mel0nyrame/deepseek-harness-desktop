...
- **打包脚本内的证据门禁** — `scripts/artifact-evidence.ts`（由 `apps/desktop/tests/package.spec.ts` 单元测试）定义了每个产出制品在管线宣告成功前都要面对的检查：`codesign --verify --deep --strict` 与针对 dmg 的 `hdiutil verify` 是硬门禁，`codesign -d` 记录身份信息，`spctl --assess` 记录 Gatekeeper 结论——只有签名身份不是 `-` 时才会强制执行，因为 macOS 对 ad-hoc 签名（即使是本地未隔离构建）一律通过 spctl 拒绝。硬门禁失败前会重试两次：刚构建出的 bundle 在繁忙 runner 上可能因签名守护进程仍在使用而让 codesign 校验报 "code has no resources but signature indicates they must be present"，安定后的复验是同一份证据。
- **跨架构交给 CI 矩阵** — `.github/workflows/release.yml` 在穷尽式 `v*` 标签 CI 成功后，于 `macos-26-intel`（x64）与 `macos-26`（arm64）上构建、签名、验证并上传 dmg。arm64 腿对工作区 `.app` 跑完整打包应用套件；x64 腿只跑制品门禁加 keyless 场景——Intel 虚拟机的焦点编排会挂起 GUI 旅程，而 keyless 场景正是该腿要证明的发布标准。两条腿都会挂载 dmg 并对挂载后的制品跑 keyless 场景。只有 keyless 场景从镜像上运行：渲染器首次绘制的时序断言在只读 HFS 挂载上会闪断。构建器配置声明 `publish: null`，否则 electron-builder 的发布管理器会在 Actions runner 上自动探测 GitHub provider（环境里存在 `GITHUB_TOKEN` 即触发），在构建 dmg update-info 时索要凭据。所有证据门禁通过后，发行工作流统一负责 GitHub 上传。矩阵刻意不做成 pull-request 门禁，因为它会让私有仓库的 macOS runner 分钟数翻倍。
