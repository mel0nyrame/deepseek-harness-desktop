...
- **跨架构交给 CI 矩阵** — `.github/workflows/desktop-release.yml` 在 `macos-26-intel`（x64）与 `macos-26`（arm64）上构建、签名、验证、挂载 dmg、对挂载后的制品冒烟并上传 dmg，由 workflow_dispatch 与 `v*` 标签触发。构建器配置声明 `publish: never`——dmg 由 CI 作为运行产物上传——因为 electron-builder 在 Actions runner 上会自动探测 GitHub provider，否则会索要发布 token。矩阵刻意不做成 pull-request 门禁：现有 PR lane 已在一台 arm64 runner 上跑打包冒烟，矩阵会让私有仓库的 macOS runner 分钟数翻倍。
