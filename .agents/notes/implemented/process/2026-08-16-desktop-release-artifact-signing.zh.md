...
- **跨架构交给 CI 矩阵** — `.github/workflows/desktop-release.yml` 在 `macos-26-intel`（x64）与 `macos-26`（arm64）上构建、签名、验证、冒烟测试并上传 dmg，由 workflow_dispatch 与 `v*` 标签触发。刻意不做成 pull-request 门禁：现有 PR lane 已在一台 arm64 runner 上跑打包冒烟，矩阵会让私有仓库的 macOS runner 分钟数翻倍。
