...
<<<<<<< HEAD
- **Symmetric cache** (restore and save): `actions/setup-node` with `cache: pnpm` — `docs-pages.yml`, `build-exe-for-python-sdk.yml`, and the node-compat and two benchmark jobs of `ci.yml`. The larger-runner benchmark keeps its store cache Linux-only through a conditional `cache:` input; the consolidated benchmark caches on both platforms.
