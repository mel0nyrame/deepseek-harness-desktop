...
- **Ad-hoc signing in the pipeline** — `electron-builder.yml` sets `identity: '-'` (ad-hoc, no Apple credentials) and `hardenedRuntime: false` (the hardened runtime would reject the `ELECTRON_RUN_AS_NODE` child loading the rebuilt node-pty addon without a `disable-library-validation` entitlement). Targets are now `dmg` and `dir`. A future Developer ID setup fills the identity name through `DSH_DESKTOP_SIGN_IDENTITY` and the Apple credentials through Actions secrets (see the [staged-CI decision](2026-09-02-staged-ci-and-main-promotion.md)); the committed yml stays credential-free.
- **Evidence gate in the package script** — `scripts/artifact-evidence.ts` (unit-tested by `tests/desktop-package.test.ts`) defines the checks every produced artifact faces before the pipeline reports success: `codesign --verify --deep --strict` and `hdiutil verify` for the dmg are hard gates, `codesign -d` records the identity, and `spctl --assess` records the Gatekeeper verdict — enforced only when a Developer ID identity (not `-`) signs, because macOS rejects every ad-hoc signature via spctl even for an unquarantined local build. The hard gates retry twice before failing: a freshly built bundle can fail codesign verification with "code has no resources but signature indicates they must be present" while the signing daemon still touches it on a busy runner, and a settled re-verification is the same evidence.
- **Cross-arch via CI matrix** — `.github/workflows/release.yml` builds both architectures natively (arm64 on `macos-15`, x64 on `macos-15-intel`), and each leg runs the same installed-application smoke against its artifact; the [staged-CI decision](2026-09-02-staged-ci-and-main-promotion.md) owns the release tiers and their secret boundaries. `publish: null` stays in the builder config because electron-builder's publish manager otherwise auto-detects the GitHub provider on Actions runners (a `GITHUB_TOKEN` in the environment is enough) and demands credentials while building the dmg update info. The release workflow owns GitHub upload after all evidence gates pass.
...
- Local `package` runs produce a signed `.app` plus `.dmg` and fail loudly when signature or image verification regresses; default ad-hoc builds keep the Gatekeeper verdict as recorded evidence, and a Developer ID identity supplied through `DSH_DESKTOP_SIGN_IDENTITY` at release time turns it into a hard gate.
