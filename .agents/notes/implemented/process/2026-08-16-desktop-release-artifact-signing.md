...
- **Cross-arch via CI matrix** — `.github/workflows/desktop-release.yml` builds, signs, verifies, smoke-tests, and uploads the dmg on `macos-26-intel` (x64) and `macos-26` (arm64), triggered by workflow_dispatch and `v*` tags. It is deliberately not a pull-request gate: the existing PR lane already smokes one arm64 runner, and the matrix doubles macOS runner minutes on a private repository.
