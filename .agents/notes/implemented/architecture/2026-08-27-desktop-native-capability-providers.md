...
- Handler removal and supervisor shutdown abort every action owned by that handler or child, then await handler promises up to the configured shutdown bound. Native adapters normally settle before the bound; an Electron operation that cannot be cancelled cannot hold application shutdown forever, and its late settlement is still discarded.
