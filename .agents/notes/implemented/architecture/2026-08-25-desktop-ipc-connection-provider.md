...
The profile retains the official Connection row because its browser bundle supplies `@deepseek-ai/dsh-client-connection/client` to the client module table. Without WebServer, that row's Host plugin remains unresolved and registers no route or listener. The `desktop-connection` row declares the official client module as an external and supplies `ctx.connection` on both sides.
