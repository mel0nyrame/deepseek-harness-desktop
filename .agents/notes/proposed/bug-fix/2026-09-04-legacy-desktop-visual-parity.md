...
| Workspace home: light, expanded, selected workspace | Approved repository image plus legacy implementation | Shared chrome, brand, panel control, row structure, and initial dimensions are restored by #101; later surface-specific tickets retain their own gaps | Deterministic real-product reference capture plus semantic and geometry assertions |
| Workspace home: no workspace selected | Legacy behavior | Shared chrome is restored; the remaining state-dependent model and permission presentation still needs its surface review | Real product journey before workspace selection |
| Sidebar: expanded, collapsed, narrow, full screen | Legacy layout and native presentation | #101 restores zero-width collapse, narrow manual reopen, full-screen control placement, and header clearance | Semantic, responsive geometry, and focused region assertions |
...
| Conversation: streaming, complete, and error | Legacy replay journey and header-clearance rules | #101 covers shared header and tab clearance; content and error-region geometry still needs its surface review | Keyless real turns plus header, tabs, content, and error-region assertions |
...
| Focus, drag, minimize, restore, and resize | Legacy native-window evidence; current traffic-light coordinates and window options remain equivalent | Shared DOM chrome is restored; inactive-material alignment remains in the appearance review | Existing OS behavior assertions plus product-owned region captures |
...
Issue #100 establishes this reference seam in the source and installed-product journeys. An isolated tracer profile supplies three reference replay scripts, one supplemental terminal-behavior script, and fixed locale and appearance preferences without touching the normal profile. The journey records the reference session identities, non-blank state, replay markers, stable ordering, animation and compositor settlement, semantic facts, resolved geometry, and image hashes in `evidence.json`. Issue #101 removes the four recorded shared-chrome mismatches: the desktop now uses the published wordmark and panel primitive in separate rows, opens at the approved size, and exercises expanded, zero-width collapsed, narrow auto-collapsed, narrow manually reopened, resized, and full-screen states with control and conversation-header geometry assertions. The only exact-version addition is a one-attribute sidebar patch that exposes a stable header hook; its manifest entry records coverage and the deletion condition. The supplemental script preserves the installed-product streaming, tool, completion, and replay-exhaustion gate without altering the three-session reference capture. The synthetic Electron fixture is classified only as a contribution composition smoke test.
