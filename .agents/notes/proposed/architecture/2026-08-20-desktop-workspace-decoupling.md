...
- `legacy/` holds the frozen monorepo: its own `package.json`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, `AGENTS.md`, `.agents/`, `.github/`, `docs/`, `packages/`, `apps/`, `vendor/`, and every other pre-decoupling root file, including the original `README.md`/`README.zh.md`/`README.i18n.yaml` trio, `CLAUDE.md`, `LICENSE`, `SECURITY.md`, and the `assets/readme/` tree as frozen duplicates. The only pre-decoupling root entries not carried over are the outdated `.codegraph/` database and the three community QR images (`assets/community-*.png`), removed as obsolete artifacts. `legacy/` is preserved for comparison and recovery until the decoupling removes it; the `legacy` branch is its recovery home. Never edit, build, or depend on it from the product workspace.
