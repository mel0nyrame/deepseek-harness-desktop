...
- The development tracer bullet runs via `pnpm run dev:desktop`: the Electron shell ([`apps/desktop`](../../../../apps/desktop)) supervises one dedicated DSH child (`--profile desktop`, the shipped `base + web-app + desktop-app` overlay). The overlay ([`packages/bundle/desktop-app`](../../../../packages/bundle/desktop-app)) disables every browser transport row (`web-startup`, `webserver`, `web-runtime`, `client-hmr`), mounts the native-picker client half, and supplies the picker and path opener through the child runtime — no loopback HTTP listener participates.
...
- `pnpm --filter @deepseek-ai/dsh-desktop run package` assembles an ad-hoc signed macOS bundle and dmg for the host architecture through six stages ([`scripts/package.ts`](../../../../apps/desktop/scripts/package.ts)): closure verification (`verify-runtime-closure` now checks both deploy manifests), a pnpm legacy deploy of the production runtime closure into a symlink-free staging directory, an Electron-ABI rebuild of node-pty validated inside the Electron binary, the electron-builder assembly and ad-hoc signing ([`electron-builder.yml`](../../../../apps/desktop/electron-builder.yml)), and the signature/Gatekeeper/image evidence gate ([`artifact-evidence.ts`](../../../../apps/desktop/scripts/artifact-evidence.ts)). Cross-arch artifacts come from the [`desktop-release.yml`](../../../../.github/workflows/desktop-release.yml) CI matrix.
