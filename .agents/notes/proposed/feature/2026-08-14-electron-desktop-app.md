...
Electron's `dialog.showOpenDialog` exposes no programmatic close or abort API, so cancellation settles an in-flight native operation logically while a dialog already on screen stays until the user dismisses it.
...
`runtime/runtime-manifest.json` and `scripts/assemble-runtime.ts` assemble from `@dsh-desktop/shell`, verify the shell and bundle entrypoints alongside official DSH and native artifacts, and materialize copied `workspace:*` specifiers to the installed desktop package versions without mutating source manifests. `tests/desktop-runtime.e2e.test.ts` launches the real Electron application over that assembled closure, creates a Session, observes an ordered recorded model turn that runs the real bash/PTY path and displays `TERMINAL_OK` plus `DONE`, reconstructs the input and tool result from Session history, proves the DSH child has no TCP listener, and verifies quiescence after normal quit, quit during startup, and a twice-failed configuration startup. With `DSH_DESKTOP_PROCESS_EVIDENCE=1`, the E2E records each owned root/descendant PID and `lstart`, then asserts none is alive after Electron exits; the original keyless tracer records the DSH root, the startup-quit scenario records the startup root, and the twice-failed configuration scenario records two generation roots. Its startup and tracer status surface uses the centered system-font hierarchy of the desktop lifecycle page; background material remains owned by issue #69's native-window work. Focused supervisor and process-tree tests pin readiness timeout, invalid configuration, unexpected exit, one controlled restart, SIGTERM/SIGKILL process-tree cleanup, and terminate-and-join behavior. Real macOS process tests cover graceful cleanup, forced escalation, a reparented root-group descendant, a live PTY, and recovery of a distinct-group PTY from the pre-exit snapshot.

Issue #67 acceptance is recorded on macOS arm64 with `pnpm run check`: typecheck, lint, every workspace build, and all 75 tests in 12 files passed. That run includes the focused supervisor, renderer-policy, and process-tree suites, the five-scenario real macOS process test, and the three-scenario real-composition Electron E2E.

Issues #1 and #2 shipped the first vertical slice through the development path:

- The reusable Client Connection carrier contract ([`carrier-contract.client.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/packages/client/connection/tests/carrier-contract.client.ts)) locks unary, reverse-response, mux-stream, and host-stream semantics plus readiness, ordering, cancellation, malformed-message, disconnect, and subscription-lifetime behavior; both the HTTP/WebSocket carrier and the new Electron carrier pass it unchanged.
- The development tracer bullet runs via `pnpm run dev:desktop`: the Electron shell ([`apps/desktop`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop)) supervises one dedicated DSH child (`--profile desktop`, the shipped `base + web-app + desktop-app` overlay). The overlay ([`packages/bundle/desktop-app`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/packages/bundle/desktop-app)) disables every browser transport row (`web-startup`, `webserver`, `web-runtime`, `client-hmr`), mounts the native-picker client half, and supplies the picker and path opener through the child runtime — no loopback HTTP listener participates.
- The renderer reaches DSH only through the sandboxed, context-isolated preload bridge; [`DesktopApiClient`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/packages/client/connection/src/client/desktop-api-client.ts) implements the existing `IApiClient` surface over it. The Host-side Connection and client-modules plugins mount their Web transports only when a WebServer is present, so the Web development workflow is unchanged.
- A keyless real-composition e2e ([`apps/desktop/tests/real-composition.e2e.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/tests/real-composition.e2e.ts)) forks the real desktop profile, creates a Session, replays a recorded `bash` tool turn, asserts the ordered streamed `TERMINAL_OK` result through the mux stream, and verifies terminate-and-join quit leaves no descendant process.

Issue #3 shipped the packaged application slice:

- `pnpm --filter @deepseek-ai/dsh-desktop run package` assembles an ad-hoc signed macOS bundle and dmg for the host architecture through six stages ([`scripts/package.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/scripts/package.ts)): closure verification (`verify-runtime-closure` now checks both deploy manifests), a pnpm legacy deploy of the production runtime closure into a symlink-free staging directory, an Electron-ABI rebuild of node-pty validated inside the Electron binary, the electron-builder assembly and ad-hoc signing ([`electron-builder.yml`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/electron-builder.yml)), and the signature/Gatekeeper/image evidence gate ([`artifact-evidence.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/scripts/artifact-evidence.ts)). Cross-arch artifacts come from the [`release.yml`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/.github/workflows/release.yml) CI matrix.
- The installed layout keeps the shell in the asar and the whole runtime closure as real files under `Contents/Resources/runtime/`; Electron main forks the application binary itself as the DSH child (`ELECTRON_RUN_AS_NODE`), resolves the CLI, Web dist, and PTY helper from that closure, and hands the child its user-data directory ([`packaged-runtime.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/src/packaged-runtime.ts) owns the contract). No system Node.js or DSH CLI participates.
...
- Real coverage: [`process-tree.spec.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/tests/process-tree.spec.ts) and [`lifecycle.spec.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/tests/lifecycle.spec.ts) drive deterministic tables and fake spawns; [`process-tree.e2e.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/tests/process-tree.e2e.ts) proves graceful termination, SIGTERM-immune escalation, reparented-descendant and orphan sweeps, and real node-pty cleanup on macOS; [`recovery.e2e.ts`](https://github.com/mel0nyrame/deepseek-harness-desktop/blob/0971b9f0e3e9293e3f76c45b1d72f5789244ccdf/legacy/apps/desktop/tests/recovery.e2e.ts) walks the development recovery journey; the packaged `--record-recovery` acceptance records startup-failed, restarting, session-recovered, and tracer-settled frames and asserts no surviving installed-app process.

Issue #8 shipped the native desktop-action slice:

- The desktop child now provides `directoryPicker` and `nativePathOpener` through one closed reverse-request union: `pick-directory` and `open-path`. Electron main validates every request, accepts only non-empty absolute paths without NUL bytes, checks target availability, invokes `dialog` or `shell.openPath`, and owns the actionable native failure dialog; the renderer still receives only the existing task-level Workspace and Host APIs.
- Cancellation follows the owning task from the renderer's `AbortSignal` through `host.pickDirectory`, the desktop child correlation, and the main-process handler. Malformed requests, duplicate ids, unavailable targets, child disconnect, handler removal, assembly disposal, and application shutdown release their correlations deterministically; late native settlements cannot revive a completed operation.
- The desktop profile mounts the renderless native-picker client half while the child runtime supplies the capability, so Web deployments retain their existing native/browse compositions. The ApiProxy path opener accepts the product-shell adapter without moving Host path resolution or business dispatch into Electron main.
- Focused handler, protocol, supervisor, client-runtime, and renderless-flow tests pin validation and cleanup. The installed `--record-native-actions` journey keeps the real window, preload, ApiProxy, reverse IPC, Workspace adoption, and Session navigation, substitutes only deterministic dialog/shell primitives, and records the selected Workspace plus successful opening and an unavailable-path failure; the Workspace discovery uses the Host's canonical realpath rather than the picker's raw macOS alias.

Issue #6 shipped interaction parity and is closed. Issue #9 shipped the ad-hoc signing tier ([release-signing note](../../implemented/process/2026-08-16-desktop-release-artifact-signing.md)); Developer ID notarization is tracked as issue #28, blocked on Apple Developer Program credentials.
