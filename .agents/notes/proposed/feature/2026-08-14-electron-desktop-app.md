...
- The development tracer bullet runs via `pnpm run dev:desktop`: the Electron shell ([`apps/desktop`](../../../../legacy/apps/desktop)) supervises one dedicated DSH child (`--profile desktop`, the shipped `base + web-app + desktop-app` overlay). The overlay ([`packages/bundle/desktop-app`](../../../../legacy/packages/bundle/desktop-app)) disables every browser transport row (`web-startup`, `webserver`, `web-runtime`, `client-hmr`), mounts the native-picker client half, and supplies the picker and path opener through the child runtime — no loopback HTTP listener participates.
- The renderer reaches DSH only through the sandboxed, context-isolated preload bridge; [`DesktopApiClient`](../../../../legacy/packages/client/connection/src/client/desktop-api-client.ts) implements the existing `IApiClient` surface over it. The Host-side Connection and client-modules plugins mount their Web transports only when a WebServer is present, so the Web development workflow is unchanged.
- A keyless real-composition e2e ([`apps/desktop/tests/real-composition.e2e.ts`](../../../../legacy/apps/desktop/tests/real-composition.e2e.ts)) forks the real desktop profile, creates a Session, replays a recorded `bash` tool turn, asserts the ordered streamed `TERMINAL_OK` result through the mux stream, and verifies terminate-and-join quit leaves no descendant process.

Issue #3 shipped the packaged application slice:

- `pnpm --filter @deepseek-ai/dsh-desktop run package` assembles an ad-hoc signed macOS bundle and dmg for the host architecture through six stages ([`scripts/package.ts`](../../../../legacy/apps/desktop/scripts/package.ts)): closure verification (`verify-runtime-closure` now checks both deploy manifests), a pnpm legacy deploy of the production runtime closure into a symlink-free staging directory, an Electron-ABI rebuild of node-pty validated inside the Electron binary, the electron-builder assembly and ad-hoc signing ([`electron-builder.yml`](../../../../legacy/apps/desktop/electron-builder.yml)), and the signature/Gatekeeper/image evidence gate ([`artifact-evidence.ts`](../../../../legacy/apps/desktop/scripts/artifact-evidence.ts)). Cross-arch artifacts come from the [`release.yml`](../../../../legacy/.github/workflows/release.yml) CI matrix.
- The installed layout keeps the shell in the asar and the whole runtime closure as real files under `Contents/Resources/runtime/`; Electron main forks the application binary itself as the DSH child (`ELECTRON_RUN_AS_NODE`), resolves the CLI, Web dist, and PTY helper from that closure, and hands the child its user-data directory ([`packaged-runtime.ts`](../../../../legacy/apps/desktop/src/packaged-runtime.ts) owns the contract). No system Node.js or DSH CLI participates.
...
- Real coverage: [`process-tree.spec.ts`](../../../../legacy/apps/desktop/tests/process-tree.spec.ts) and [`lifecycle.spec.ts`](../../../../legacy/apps/desktop/tests/lifecycle.spec.ts) drive deterministic tables and fake spawns; [`process-tree.e2e.ts`](../../../../legacy/apps/desktop/tests/process-tree.e2e.ts) proves graceful termination, SIGTERM-immune escalation, reparented-descendant and orphan sweeps, and real node-pty cleanup on macOS; [`recovery.e2e.ts`](../../../../legacy/apps/desktop/tests/recovery.e2e.ts) walks the development recovery journey; the packaged `--record-recovery` acceptance records startup-failed, restarting, session-recovered, and tracer-settled frames and asserts no surviving installed-app process.
