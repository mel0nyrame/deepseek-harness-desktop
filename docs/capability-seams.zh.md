...
flowchart LR
  ...
  svc_nativePathOpener["ctx.nativePathOpener<br/>Product-shell path opening seam"]
  ...
  pkg_apiproxy --> svc_nativePathOpener
  ...
  pkg_desktop_app --> svc_directoryPicker
  pkg_desktop_app --> svc_nativePathOpener
  ...
  svc_nativePathOpener --> pkg_apiproxy
...
| `ctx.directoryPicker` | `seam` | `directory-picker` | `directory-picker-native`, `directory-picker-browse`, [`desktop-app`](../packages/bundle/desktop-app) | `apiproxy` | - | 带判别标记的交互能力：原生后端在 Host 显示设备上打开一个操作系统选择器，浏览后端为应用内浏览器提供列表与创建原语；双端后端通过其浏览器侧填充 ui-workspace 目录流程的 slot（不通过协议发布）。 |
| `ctx.nativePathOpener` | `seam` | `apiproxy` | [`desktop-app`](../packages/bundle/desktop-app) | `apiproxy` | - | 网关按 Host 工作目录解析相对路径；组合后的产品 shell 报告当前可用性，并在调用方生命周期内执行用户可见的桌面交接。 |
