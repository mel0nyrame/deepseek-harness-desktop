...
<<<<<<< HEAD
The package also exports the Host-only `./sidebar-glass` contribution used by the macOS desktop composition. That row registers `ui-sidebar-glass-macos.enabled`, whose default is `true`; Web and non-macOS desktop compositions do not register the namespace. General → Appearance renders the switch only after that namespace is ready and writable, so a Darwin renderer without the contribution cannot expose a non-functional default. The saved value projects onto one of `glass-light`, `glass-dark`, `opaque-light`, or `opaque-dark`, then writes through the Host settings scope. Reduce Transparency selects the theme-matched opaque material without changing the saved value, so restoring system transparency or restarting the application recovers the user's preference. The material adapter writes only `body[data-dsh-sidebar-material]`; the desktop shell owns the native vibrancy and surface CSS.
