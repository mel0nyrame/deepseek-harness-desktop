...
describe('sidebar shell snapshots', () => {
  // The expanded shell carries the inline wordmark, the New Session capsule,
  // and the CSS-hidden compact brand-row seam (the desktop shell reveals the
  // seam under body[data-dsh-platform='darwin']; the un-styled test DOM
  // cannot apply that hiding, so the seam button counts here).
  const NEW_SESSION_BUTTONS = 3
  ...
  it('renders the expanded column in the default locale (zh, no setLocale)', async () => {
    ...
    expect(slot.view.getAllByRole('button', { name: '新建会话' })).toHaveLength(NEW_SESSION_BUTTONS)
  ...
  it('renders the expanded column (wordmark, capsule, empty holes)', async () => {
    ...
    const slot = runtime.renderSlot('sidebar', { collapsed: false, width: 300 })
    // Wordmark + capsule both start a session in the expanded state.
    expect(slot.view.getAllByRole('button', { name: 'New session' })).toHaveLength(NEW_SESSION_BUTTONS)
    expect(slot.container).toMatchSnapshot()
    await runtime.dispose()
  })
