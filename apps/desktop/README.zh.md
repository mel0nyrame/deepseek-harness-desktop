...
macOS `BrowserWindow` 使用 `hiddenInset` 标题栏、固定内嵌位置的 traffic lights、透明客户端表面，以及 Electron 基于 AppKit 的 `under-window` vibrancy，并将 `visualEffectState` 设为 `followWindow`。紧凑窗口框架移除了合成的全宽标题区域：客户端根节点填满整个窗口；在 macOS 上，侧边栏第一行与原生 traffic lights 共用顶部区域，侧边栏开关位于原生灯组右侧并留出 16 像素间距，DeepSeek Harness 字标位于下一行。紧凑头部是仅 macOS 的呈现；Web 与其他桌面平台保留原有侧边栏外壳。处于最上层的对话头部、侧边栏控制行与字标行、以及模态框标题行可拖动原生窗口；链接、表单控件、按钮、可编辑内容、侧边栏工作区列表、对话正文、输入区与模态框正文保持为可交互的 no-drag 区域。原生全屏中，AppKit 会自动隐藏 traffic lights，并在指针到达屏幕顶端时重新显示；应用不会强制控制其显隐。侧边栏控制行与字标行会移到侧边栏左侧内容内缩位置，且不会纵向重排其余侧边栏内容。收起侧边栏会解析为零宽度轨道：侧边栏材质、内容、缩放手柄与分隔线全部消失，对话区收回完整宽度；框架的展开控件——窗口态位于 traffic lights 右侧，全屏时位于左侧内容内缩位置——保持在侧边栏子树之外；对话头部只避开这个左上角 chrome 簇，让标题与展开控件对齐，并使 Chat / Trajectory 行及分割线随头部上移，绝不保留原侧边栏宽度。展开时恢复最后一次可用宽度。

展开后的 macOS 侧栏使用一块连续的原生材质，会话与详情表面保持不透明。浅色与深色玻璃变体都让侧栏在现有 AppKit 材质上保持透明；context-isolated preload bridge 会把应用偏好镜像到 Electron 经过校验的 main-process IPC，由 `nativeTheme.themeSource` 选择匹配的原生 Light、Dark 或 System 外观。深色 `under-window` 材质路径保持不变。New Session 与选中会话在两种玻璃变体上都使用清晰可读的半透明语义覆盖层。General → Appearance 只在 macOS Host namespace 存在且可写时显示默认开启的 `Sidebar glass effect` 开关；操作会立即改变材质，并通过 Host 持久化 `ui-sidebar-glass-macos.enabled`，包括跨应用重启保留。“减少透明度”会强制匹配主题的不透明侧栏 fallback，但不会改写保存的开关；恢复透明度后，只要偏好仍然开启就会再次显示玻璃。Electron 支持的接口已满足布局与无障碍约定，因此应用不包含自定义原生视觉效果 addon。

## 打包产物验收

`apps/desktop/tests/packaged-smoke.e2e.ts` 以九种模式启动安装后的应用包。`--inspect-native-window` 创建真实 `BrowserWindow`，并报告标题栏、traffic lights、透明度、vibrancy、焦点、拖动区域、外观与降低透明度状态，供自动化断言。`--accept-native-window` 在装配好的渲染器上打开可见窗口，断言 active → inactive → active 焦点切换、最小化/恢复、原生全屏进入/退出与侧边栏控制对齐、窗口态控制行避开 traffic lights 的内缩、合成的拖动区域输入尝试、紧凑全高布局且内容不被遮挡、计算得到的 drag/no-drag 区域，以及真实输入框的键盘路径。随后它把侧边栏拖到自定义宽度并收起，断言零宽度网格轨道、被收回的对话区、位于 traffic lights 右侧的展开控件、收起态头部的避让与顶部内缩、全屏时展开控件对齐左侧内容内缩，以及恢复的正是拖拽宽度。`--accept-native-window-drag --smoke-replay <file>` 是通过 `DSH_DESKTOP_NATIVE_DRAG_REQUIRED=1` 选择性启用的操作系统指针验收通道：外部 CoreGraphics 驱动器分别拖动引导标题、展开态侧边栏控制行与收起态对话头部，并通过原生 `BrowserWindow` 边界验证每次移动；随后验证输入区保持不动并获得焦点。macOS Accessibility 权限不可用时，该通道会硬失败。`--accept-sidebar-glass --sidebar-glass-phase <default-off|reopen-on|reopen-enabled>` 使用一个显式测试 home 连续启动三次：断言默认开启的玻璃与不透明内容表面、即时关闭及 Host 持久化、重启后恢复关闭状态、重新开启、透明的明暗玻璃、从 Electron main 读取并匹配的 `nativeTheme.themeSource` 值、“减少透明度”启用时使用不透明 fallback 且不丢失偏好，以及最终重启后原生浅色玻璃仍保持开启。`--record-native-window --smoke-replay <file> [--smoke-child-replay <file> …]` 配合 `DSH_DESKTOP_FRAMES_DIR` 录制真实渲染器帧：启动、装配后的原生拖动表面、焦点切换、拖动区域尝试、键盘操作、最小化/恢复、原生全屏切换（展开与收起两种状态）、零宽度收起侧边栏、展开后的侧边栏、带标题、标签与分割线的已完成会话收起帧、明暗外观、装配 UI 中回放的 terminal 回合，以及通过装配好的提问面板与审批面板作答的提问回合与审批回合，并在结束后把 `nativeTheme.themeSource` 恢复为进入录制模式时的值。`--record-native-actions` 配合 `DSH_DESKTOP_FRAMES_DIR` 保留真实安装态窗口、preload、ApiProxy、子进程反向 IPC、Workspace 接纳与 Session 导航，只替换为确定性的对话框／shell primitive；它会记录已接纳 Workspace 及结构化的成功与可操作失败证据，因为 renderer capture 无法包含原生对话框。`--record-recovery --smoke-replay <file>` 配合 `DSH_DESKTOP_FRAMES_DIR` 会植入损坏的 desktop profile，录制失败状态页、受控重启与恢复后装配 UI 中回放 terminal 回合的真实渲染器帧，随后干净退出。`--smoke --smoke-replay <file> [--smoke-child-replay <file> …]` 断言完整的无密钥交互等价场景——持久 Workspace 的创建与幂等重开、由终端执行的 `echo TERMINAL_OK` 工具回合及其有序流式事件、经载体作答的提问回合与沙箱升级审批回合、从持久 Session 日志重建全部模型可见输入、无 TCP 监听、干净退出——并确认没有残留的自有进程。第二次以 `--smoke-reopen --smoke-home <dir>` 启动同一持久主目录，断言 Workspace 与三个 Session 无需任何模型调用即从现有持久化中重建。应用包缺失时该用例自跳过；macOS CI 任务会先打包并设置 `DSH_DESKTOP_SMOKE_REQUIRED=1`，把缺失变成硬失败。所有旅程模式都会在 `ready` 之前禁用硬件加速：CI 的 macOS runner 没有可靠的 GPU，瞬时 command-buffer 失败会卡死渲染器的脚本通道、让旅程的 `executeJavaScript` 等待挂起，软件合成则让每次旅程的绘制与 `capturePage` 帧在任何主机上都保持确定。
...
- `capturePage()` 只能看到渲染器像素：录制的帧不包含原生 traffic lights 图形，合成输入也无法证明原生窗口发生移动。选择性启用的 CoreGraphics 验收通道提供操作系统指针证据；具备屏幕录制权限的机器可以用系统级捕获替换 renderer 帧，而断言无需改动。
